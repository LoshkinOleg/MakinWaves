...
constexpr const float WAVE_CELERITY_X = 24.0f; // Speed at which a wave propagates in the X coordinate (speed of sound). Too slow and the simulation breaks down since the source oscilates faster than the speed of sound. Too fast and the simulation breaks down as well.
constexpr const float WAVE_CELERITY_Y = 24.0f; // 24 seems to be the sweet spot for this setup.
...
constexpr const float SOURCE_AMPLITUDE = 1000000000.0f; // Needs to be very big to see anything due to the dt^2 factor when computing the effect of the forcing function.
...
int main(void)
{
    ...
    Sim::Application app =
        Sim::Application(
            ...
            DT, WAVE_CELERITY_X, WAVE_CELERITY_Y, SIMULATION_GRID_RESOLUTION_X, SIMULATION_GRID_RESOLUTION_Y, BOUNDARY_CONDITION, // Simulation parameters.
