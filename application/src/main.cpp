...
static constexpr const double FPS = 120.0f;
static constexpr const double RESOLUTION = 80.0f;
static constexpr const double CELERITY = 343.0f * 0.001f; // ~1/3 of a meter per second
static constexpr const double CFL = 2.0f * (CELERITY * (1.0f / FPS) / (1.0f / RESOLUTION));

// Source parameters.
static constexpr const double Pi = 3.14159265359;
static constexpr const double FREQUENCY = 3; // 3 hertz
static constexpr const double AMPLITUDE = 1.0;
...
int main(void)
{
    ...
    ExplicitWorld2D w(CELERITY, initialConditions, FieldUpdate::HAROON, BoundaryCondition::DIRICHLET, sources, rects);

    InitWindow((int)DISPLAY_RESOLUTION, (int)DISPLAY_RESOLUTION, nullptr);
    SetTargetFPS((int)FPS / 1);
